...
import QuizWidget from './QuizWidget'; // <--- Import New Widget

type Props = {
  exams: Exam[];
  onQuizComplete: (examId: string, bonusHours: number) => void; // <--- Pass handler
};

export default function AnalyticsDashboard({ exams, onQuizComplete }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 1. Memory Drain Widget */}
      <MemoryDrainWidget exams={exams} />

      {/* 2. REPLACED: Priority Target -> Polymer Quiz Widget */}
      <QuizWidget exams={exams} onQuizComplete={onQuizComplete} />

      {/* 3. Load Balancer (Existing) */}
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm transition-colors">
        ...
        <div className="space-y-4">
          {distribution.length === 0 ? (
             <div className="text-center py-8 text-slate-400 italic">Add subjects to unlock analysis.</div>
          ) : (
